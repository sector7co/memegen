...
export async function listMemes(query = '', requestedLimit = 12) {
  ...
  const result = await env.DB.prepare(
    `SELECT m.id, m.title, m.image_key, m.created_at,
     ...
     GROUP BY m.id, m.title, m.image_key, m.created_at
...
export async function createMemeMetadata(input: {
  ...
  const statements = [
    env.DB.prepare('INSERT INTO memes (id, title, image_key, content_type, created_at) VALUES (?, ?, ?, ?, ?)')
      .bind(input.id, input.title, input.imageKey, 'image/png', Date.now()),
